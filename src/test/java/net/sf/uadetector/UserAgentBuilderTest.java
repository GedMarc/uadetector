...
public class UserAgentBuilderTest
{
	...
	{
		OperatingSystem os = new OperatingSystem(OperatingSystemFamily.UNKNOWN, "family", "icon", "name", "producer", "producer url",
		                                         "url", new VersionNumber("1"));

		UserAgent.Builder b = new UserAgent.Builder();
		...
		assertThat(b.getVersionNumber()
		            .toVersionString()).isEqualTo("1.0.0");

		UserAgent ua = b.build();
	...
	{
		ReadableOperatingSystem os = EasyMock.createMock(ReadableOperatingSystem.class);
		EasyMock.expect(os.getFamily())
		        .andReturn(OperatingSystemFamily.LINUX)
		        .anyTimes();
		EasyMock.expect(os.getFamilyName())
		        .andReturn("f1")
		        .anyTimes();
		EasyMock.expect(os.getIcon())
		        .andReturn("i1")
		        .anyTimes();
		EasyMock.expect(os.getName())
		        .andReturn("n1")
		        .anyTimes();
		EasyMock.expect(os.getProducer())
		        .andReturn("p1")
		        .anyTimes();
		EasyMock.expect(os.getProducerUrl())
		        .andReturn("pu1")
		        .anyTimes();
		EasyMock.expect(os.getUrl())
		        .andReturn("u1")
		        .anyTimes();
		EasyMock.expect(os.getVersionNumber())
		        .andReturn(new VersionNumber("1", "0"))
		        .anyTimes();
		EasyMock.replay(os);
	...
	{
		String userAgentString = "a user agent string";
		UserAgent.Builder b = new UserAgent.Builder().setUserAgentString(userAgentString);
