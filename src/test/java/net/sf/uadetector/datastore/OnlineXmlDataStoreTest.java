...
public class OnlineXmlDataStoreTest
{
	 ...
	 */
	private static final URL DATA_URL = AbstractDataStoreTest2.class.getClassLoader()
	...
	public void construct_successful()
	{
		...
		OnlineXmlDataStore store = new OnlineXmlDataStore(DATA_URL, VERSION_URL, fallbackDataStore);
	...
	public void readData_failsAndReturnsFallbackData()
	{
		...
		OnlineXmlDataStore store = new OnlineXmlDataStore(DATA_URL, VERSION_URL, fallbackDataStore);
		assertThat(store.getData()
		                .getVersion()).isEqualTo(fallbackDataStore.getData()
		                                                          .getVersion());
		assertThat(store.getDataReader()).isNotNull();
		assertThat(store.getDataUrl()).isNotNull();
		assertThat(store.getVersionUrl()
