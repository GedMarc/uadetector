...
public class CachingXmlDataStoreTest_deleteCacheFile
{
	...
	{
		File cache = createMock(File.class);
		expect(cache.canRead()).andReturn(true)
		                       .anyTimes();
		expect(cache.isFile()).andReturn(true)
		                      .anyTimes();
		// expect(cache.isInvalid()).andReturn(false).anyTimes(); // maybe necessary when using JDK7
		expect(cache.delete()).andReturn(false)
		                      .anyTimes();
		expect(cache.exists()).andReturn(true)
		                      .anyTimes();
		File tmpFile = folder.newFile();
		expect(cache.getPath()).andReturn(tmpFile.getPath())
		                       .anyTimes();
		expect(cache.toURI()).andReturn(tmpFile.toURI())
		                     .anyTimes();
		replay(cache);
