...
public class CachingXmlDataStoreTest_loadingWrongContent
{
	 ...
	 */
	private static final URL DATA_CONNECTION_ERROR_URL = TestXmlDataStore.class.getClassLoader()
	                                                                           .getResource("uas_connection_error.xml");

	/**
	 * URL to retrieve wrong content instead of UAS data as XML
	 */
	private static final URL VERSION_CONNECTION_ERROR_URL = TestXmlDataStore.class.getClassLoader()
	                                                                              .getResource(
			                                                                                               ...
			                                                                                               URISyntaxException
	{
		...
		byte[] wrongContent = ByteStreams.toByteArray(DATA_CONNECTION_ERROR_URL.openStream());
		...
		CachingXmlDataStore store = CachingXmlDataStore.createCachingXmlDataStore(cache, DATA_CONNECTION_ERROR_URL,
		                                                                          VERSION_CONNECTION_ERROR_URL, CHARSET, fallback);

		assertThat(store.getData()
		                .getVersion()).isEqualTo(TestXmlDataStore.VERSION_OLDER);
			                                                                                           ...
			                                                                                           URISyntaxException
	{
		...
		// create caching data store without a cache file
		CachingXmlDataStore store = CachingXmlDataStore.createCachingXmlDataStore(cache, TestXmlDataStore.DATA_URL_NEWER,
			                                                                                     ...
			                                                                                     URISyntaxException
	{
		...
		assertThat(Files.toString(cache, CHARSET)
		                .isEmpty()).isTrue();
